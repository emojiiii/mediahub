...
| 数据面 Policy enforcement | GetObject、HeadObject、ListObjectsV2、PutObject、CopyObject/UploadPartCopy、Multipart、Object Tagging、Object ACL、Retention/LegalHold、DeleteObject/DeleteObjects 已完成 | Identity/Bucket Deny 优先、同账户 union、跨账户 intersection、匿名读取 Allow、坏签名不降级；条件附加权限、逐项批量删除、Governance bypass、防 upload 枚举、跨账户 owner/audit 均有真实 PG17 HTTP contract | ListObjectVersions、account action 与 Bucket 配置控制面尚未消费统一服务；Copy/Multipart 显式 Object Lock header 仍明确拒绝 |
