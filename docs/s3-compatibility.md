...
| S3 Policy enforcement | 当前受支持的对象、版本、Multipart、account、Bucket 基础与 Bucket 配置 operation 均已完成；Bucket Policy 管理为 owner-only Identity Policy | Identity/Bucket Deny 优先、同账户 union、跨账户 intersection、匿名读取 Allow、坏签名不降级；条件附加权限、逐项批量删除、Governance bypass、防 upload 枚举、跨账户 owner/audit、Object Lock CreateBucket 附加权限和 Policy 不可自授权均有真实 PG17 HTTP contract | Copy/Multipart 显式 Object Lock header 仍明确拒绝；未实现的 CORS、Notification、Bucket Tagging、SSE 等没有 Policy 接面 |
...
Object Lock 矩阵只创建 GOVERNANCE retention，不创建测试时间内无法安全清理的 COMPLIANCE retention。PowerShell native-client 矩阵暂不覆盖 Bucket Policy/PolicyStatus、CORS、Notification、SSE、Lifecycle 执行效果和 virtual-host style；Bucket Policy/PolicyStatus 由 Core、PostgreSQL persistence 与真实 PG17 HTTP contract 覆盖。当前受支持的匿名/签名对象读写、Copy、Multipart、版本列表、Tagging、ACL、Retention/LegalHold、删除、account action 与 Bucket 控制面 Policy enforcement 均由独立真实 PG17 HTTP contract 覆盖。其他能力应在相应协议切片实现后以独立断言加入，不能以现有 `SKIP` 或其他操作的成功替代。
