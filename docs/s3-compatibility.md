...
| S3 Policy enforcement | 当前受支持的对象、版本、Multipart、account、Bucket 基础、CORS 与 Bucket Tagging operation 均已完成；Bucket Policy 管理为 owner-only Identity Policy | Identity/Bucket Deny 优先、同账户 union、跨账户 intersection、匿名读取 Allow、坏签名不降级；条件附加权限、逐项批量删除、Governance bypass、防 upload 枚举、跨账户 owner/audit、Object Lock CreateBucket 附加权限和 Policy 不可自授权均有真实 PG17 HTTP contract | Copy/Multipart 显式 Object Lock header 仍明确拒绝；Notification/Bucket Encryption 尚无 Policy 接面；SSE 请求头当前统一 fail-closed |
| Bucket CORS | 严格 100-rule Core/XML 模型；GET/PUT/DELETE `?cors`；PUT 强制 Content-MD5；独立 revision persistence；OPTIONS 首条规则匹配和实际响应头 | Core/协议/运行时表驱动测试、PostgreSQL 17 persistence contract、Server 全量回归 | 尚未加入 PowerShell native-client 矩阵；virtual-host style 未实现 |
| Bucket Tagging | GET/PUT/DELETE `?tagging`；独立 50-tag 上限、严格 XML/Content-MD5、`NoSuchTagSet` 与 revision persistence | Core 边界测试、Server 协议测试、PostgreSQL 17 persistence contract | 尚未加入 PowerShell AWS CLI 矩阵 |
| SSE 安全边界 | 当前不宣称 SSE-S3；Put/Get/Head/Copy/Multipart/Post/Delete 的 SSE-S3/KMS/C 与 copy-source SSE-C 头统一 fail-closed | Server guard 9/9 与全量回归 | 真正 AES-GCM 信封加密、Range/Copy/Multipart/预览/DAV 全链路仍待独立纵向切片 |
...
Object Lock 矩阵只创建 GOVERNANCE retention，不创建测试时间内无法安全清理的 COMPLIANCE retention。PowerShell native-client 矩阵暂不覆盖 Bucket Policy/PolicyStatus、Bucket CORS/Tagging、Notification、SSE、Lifecycle 执行效果和 virtual-host style；Bucket Policy、CORS 与 Bucket Tagging 目前由 Core、PostgreSQL persistence、Server 协议/运行时及真实 PG17 合同覆盖。当前受支持的匿名/签名对象读写、Copy、Multipart、版本列表、Tagging、ACL、Retention/LegalHold、删除、account action 与 Bucket 控制面 Policy enforcement 均由独立真实 PG17 HTTP contract 覆盖。其他能力应在相应协议切片实现后以独立断言加入，不能以现有 `SKIP` 或其他操作的成功替代。
