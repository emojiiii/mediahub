...
const OPERATIONS: &[OperationContract] = &[
    ...
    op!(Get "/api/v1/admin/system/version", "Read the current build and update status", Session, ["ForceVersionRefresh"], None, [ResponseContract::json("200", "AdminSystemVersion"), INVALID, UNAUTHORIZED, FORBIDDEN]),
    op!(Post "/api/v1/admin/system/update", "Request a controlled container image update", SessionCsrf, [], None, [ResponseContract::json("202", "SystemUpdateOperation"), INVALID, UNAUTHORIZED, FORBIDDEN, CONFLICT]),
    ...
    op!(Get "/api/v1/object-versions/{version_id}/preview-manifest", "Resolve an immutable ObjectVersion preview manifest", SessionOrHmac, ["ApplicationContext", "ObjectVersionId"], None, [ResponseContract::json("200", "ObjectVersionPreviewManifest"), UNAUTHORIZED, FORBIDDEN, NOT_FOUND, UNAVAILABLE]),
    op!(Get "/api/v1/object-versions/{version_id}/content", "Read immutable ObjectVersion content", SessionOrHmac, ["ApplicationContext", "ObjectVersionId", "Range", "IfNoneMatch"], None, [ResponseContract { status: "200", body: ResponseBody::Binary }, ResponseContract { status: "206", body: ResponseBody::Binary }, ResponseContract::empty("304"), UNAUTHORIZED, FORBIDDEN, NOT_FOUND, ResponseContract::empty("416"), UNAVAILABLE]),
    op!(Head "/api/v1/object-versions/{version_id}/content", "Read immutable ObjectVersion content headers", SessionOrHmac, ["ApplicationContext", "ObjectVersionId", "Range", "IfNoneMatch"], None, [ResponseContract::empty("200"), ResponseContract::empty("206"), ResponseContract::empty("304"), UNAUTHORIZED, FORBIDDEN, NOT_FOUND, ResponseContract::empty("416"), UNAVAILABLE]),
    op!(Post "/api/v1/short-links", "Create a short link for a public object", SessionCsrfOrHmac, ["ApplicationContext"], Some(RequestBody::Json("CreateShortLink")), [ResponseContract::json("201", "ShortLink"), INVALID, UNAUTHORIZED, FORBIDDEN, NOT_FOUND, CONFLICT, UNAVAILABLE]),
    ...
    op!(Get "/api/v1/access-keys/{access_key_id}/s3-policy", "Read an access key S3 Identity Policy", Session, ["AccessKeyId"], None, [ResponseContract::json("200", "S3IdentityPolicy"), UNAUTHORIZED, NOT_FOUND, UNAVAILABLE]),
    op!(Put "/api/v1/access-keys/{access_key_id}/s3-policy", "Install or replace an access key S3 Identity Policy", SessionCsrf, ["AccessKeyId"], Some(RequestBody::Json("S3IdentityPolicy")), [ResponseContract::json("200", "S3IdentityPolicy"), INVALID, UNAUTHORIZED, FORBIDDEN, NOT_FOUND, CONFLICT, TOO_LARGE, UNSUPPORTED, UNAVAILABLE]),
    op!(Delete "/api/v1/access-keys/{access_key_id}/s3-policy", "Delete an access key S3 Identity Policy", SessionCsrf, ["AccessKeyId"], None, [ResponseContract::empty("204"), UNAUTHORIZED, FORBIDDEN, NOT_FOUND, UNAVAILABLE]),
    op!(Get "/s/{code}", "Redirect a public short link", Public, ["ShortCode"], None, [ResponseContract::empty("307"), NOT_FOUND, UNAVAILABLE]),
