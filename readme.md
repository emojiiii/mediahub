...
PrismArk 目前处于快速迭代阶段。现有版本已经具备可靠上传、Local/S3 存储后端、WebDAV、S3 核心对象闭环、图片 Variant、多格式预览、应用隔离、访问密钥和 Webhook。S3 网关已经覆盖 Bucket、Put/Get/Head/List/Delete/DeleteObjects、Multipart、三态 Versioning、null version、delete marker 与持久化 GC；Policy、完整 Lifecycle 执行器、Object Lock 配置接口和更多扩展操作仍在按照 [S3 修改方案](docs/mediahub-s3-modification-plan.md) 推进。
...
├─ Lifecycle                配置 API 可用，版本执行器待补齐
└─ Object Lock              删除保护可用，配置 API 待补齐
...
| WebDAV | 可用 | 文件客户端兼容层；后续统一到新 ObjectService |
| S3 存储后端 | 可用 | PrismArk 可把二进制保存到外部 S3/R2/兼容服务 |
| S3 Gateway | 核心闭环可用 | Bucket、Put/Get/Head/List/Delete/DeleteObjects、Multipart；仍不是完整 S3 实现 |
...
| Versioning | 部分可用 | 已支持三态、null version、delete marker 和精确版本读删；缺 ListObjectVersions |
| 标准 S3 Lifecycle | 部分可用 | 配置读写与严格校验已接入；ObjectVersion 生命周期执行器尚未完整落地 |
| Object Lock | 部分可用 | 删除事务已执行 Retention/Legal Hold/Governance 检查；配置和对象级修改 API 待补齐 |
...
SDK 请配置自定义 endpoint、`us-east-1` Region 与 Path Style 寻址。Web 控制台、JSON API、WebDAV、health 和 metrics 继续使用 3000 端口；S3 端口不提供健康检查。Versioning、Delete Marker、持久化 GC 与 Multipart 已进入同一 ObjectVersion 纵向闭环；尚未支持的重点包括 CopyObject/UploadPartCopy、ListObjectVersions、标准 Policy、Tagging、Object Lock 修改接口、通知、SSE 以及更广泛的 SDK 兼容矩阵。
...
4. 对齐 Bucket、Put/Get/Head/Delete/DeleteObjects、ListObjectsV2 与 Multipart 核心路径。
5. 使用 UploadIntent、原子版本提交和持久化 GC 处理上传、覆盖与回收。
6. 通过独立 9000 Listener 提供无 `/s3` 前缀的 Path Style S3 endpoint。

下一阶段重点是 CopyObject/UploadPartCopy、ListObjectVersions、标准 Policy、Lifecycle 执行器、Object Lock 修改 API、Tagging/Notification，以及把 WebDAV、预览和 Variant 统一绑定到 ObjectService 与不可变 `object_version_id`。
