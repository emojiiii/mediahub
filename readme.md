...
Web UI 位于仓库 `web/`，由 Vite 构建并部署到 Cloudflare Pages。前端只使用生成的 API Client，不直接拼接管理 API URL。
...
- Pages 构建通过 `VITE_API_BASE_URL` 注入 API Origin，环境变量不得包含 SecretAccessKey 或主密钥。
- 控制台不包含 Mock API 或演示账号；未注入 `VITE_API_BASE_URL` 时直接访问 `http://localhost:3000`。
- React Router SPA Fallback 只作用于 Web UI，不代理或吞掉 API/媒体响应。
...
```text
app.example.com    Cloudflare Pages Web UI
api.example.com    管理 API
cdn.example.com    用户媒体内容，不携带控制台 Cookie
```

Web UI 和内容域必须隔离。HTML、SVG 等潜在可执行上传内容不能与登录控制台共享 Origin。前端通过严格 CORS Allowlist 和带凭证请求访问 API；私有媒体预览使用短时签名 URL。
